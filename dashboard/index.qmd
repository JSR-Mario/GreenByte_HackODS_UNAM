...

{
  ...
  // ── 3. Cargar datos (después del yield para evitar pérdida de contexto) ───
  const raw = await FileAttachment("resultados/master_greenbyte_v4_hotspots.csv").csv({ typed: true })

  if (!raw || raw.length === 0) {
    div.innerHTML = `<div style="color:#f8fafc;padding:2rem;text-align:center;">No se pudo cargar el archivo de hotspots.</div>`
    return
  }
