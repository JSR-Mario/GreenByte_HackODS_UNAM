...
<div id="hotspot-map" style="width:100%; height:600px; border-radius:12px; overflow:hidden; background:#0f172a;"></div>
<script>
(async () => {
  // 1. Cargar Leaflet
  ...
  // 3. Fetch CSV
  const div = document.getElementById('hotspot-map');
  let raw;
  try {
    const resp = await fetch('resultados/master_greenbyte_v4_hotspots.csv');
    if (!resp.ok) throw new Error('HTTP ' + resp.status);
    raw = parseCSV(await resp.text());
  } catch(e) {
    div.innerHTML = '<div style="color:#f8fafc;padding:2rem;text-align:center;">Error: ' + e.message + '</div>';
    return;
  }
  if (!raw || raw.length === 0) {
    div.innerHTML = '<div style="color:#f8fafc;padding:2rem;text-align:center;">Sin datos.</div>';
    return;
  }
  ...
  const ms = raw.map(d => +d.multi_stress).filter(v => isFinite(v)).sort((a, b) => a - b);
  ...
  for (const row of df) {
    ...
    L.circleMarker([lat, lon], {
      ...
      fillOpacity: 0.85, weight: hot ? 1.5 : 0.8
    }).bindPopup(
  ...
  legend.onAdd = () => {
    ...
    d.style.cssText = 'background:rgba(15,23,42,0.9);color:#f8fafc;padding:10px 14px;border-radius:8px;border:1px solid #475569;font-size:13px;line-height:1.8;';
    d.innerHTML = '<b style="color:#22c55e">Simbología</b><br>' +
